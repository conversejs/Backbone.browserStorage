(function (root, factory) {
    ...
    function IndexedDBDriver (schema, ready, nolog, onerror) {
        this.schema = schema;
        this.ready = ready;
        this.error = null;
        ...
        this.dbRequest.onupgradeneeded = (event) => {
            ...
            if (!this.nolog) {
                debugLog("onupgradeneeded = " + oldVersion + " => " + newVersion);
            }
    ...
    function ExecutionQueue(schema, next, nolog) {
        this.driver = new IndexedDBDriver(schema, this.ready.bind(this), nolog, this.error.bind(this));
        this.started = false;
        this.failed = false;
        this.stack = [];
    ...
    // Method used by Backbone to sync data with data store. It was initially designed to work with "server side" APIs, This wrapper makes
    ...
    const indexedDBSync = function (method, object, options) {
        if (method == "closeall") {
        ...
        var schema = object.browserStorage.database;
        options.storeName = options.storeName || object.browserStorage.storeName;
